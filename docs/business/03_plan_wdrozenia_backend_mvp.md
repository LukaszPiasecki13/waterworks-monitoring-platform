...
- **Brak osobnego modelu/endpointu `DeviceDiagnostic`.** Zdarzenia diagnostyczne firmware trafiają jako pole `errors[]` w tym samym pakiecie telemetrycznym (zapisywane w `telemetry_errors`, indeks `(device_id, code, occurred_at)`) — nie jako osobny `POST /telemetry/diagnostics` z odrębnym modelem co 15 minut, jak zakładał pierwotny format wiadomości diagnostycznej z `01_plan_biznesowy.md` §3.4.3.
- **Stan urządzenia dojechał później, jako kanał sekcyjny (B-08).** Model nazywa się `DeviceStateReport`, nie `DeviceDiagnostic`, i jest agnostyczny wobec treści: jeden wiersz na parę (pakiet, sekcja), sekcja `device` jako pierwszy odczyt. Wybór „blok w istniejącym POST-cie zamiast osobnego endpointu" jest uzasadniony rachunkiem transferu (418 B danych vs ~5 KB narzutu TLS na osobne żądanie) w [`01_kanal_stanu_urzadzenia.md`](../technical/01_kanal_stanu_urzadzenia.md). Przy okazji rozdzielono `Device.last_seen_at` (każdy pakiet) od `Device.last_diagnostics_at` (pakiet z sekcją stanu) — do tej pory drugie pole znaczyło to, co pierwsze.
- **Tabela `telemetry_errors` nie miała migracji.** Model istniał od Etapu 2, ale żadna rewizja Alembica go nie tworzyła, więc każdy pakiet z niepustym `errors[]` kończył się błędem zapisu. Autogeneracja migracji przy B-08 wyłapała to i naprawia razem z `device_state_reports`.
...
- [x] Pozwala rozróżnić awarię infrastruktury od awarii telemetryki — zdarzenia przez pole `errors[]` pakietu, stan gatewaya (RSSI, uptime, restarty, zapełnienie bufora) przez sekcję `state[]` tego samego pakietu (B-08, nie osobny endpoint — patrz Etap 2) + status `no_comm` vs. `out_of_range`/`sensor_error` w danych (jakość pomiaru).
